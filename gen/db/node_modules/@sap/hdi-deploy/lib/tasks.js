...
const {checksum, callbackTimeout, getUniqueAndDuplicatesByKey} = require('./utils');
...
function getParameters (options, logger) {
  ...
  const [deployParameters, overwrittenParameters] = getUniqueAndDuplicatesByKey(all_parameters, (item) => item.key);
  if (overwrittenParameters.length > 0) {
    let warnMessage = '';
    overwrittenParameters.forEach(item => {
      warnMessage = `${warnMessage}${item.key} : ${item.value} `;
    });
    logger.warn(`Command line parameters : [ ${warnMessage} ] are overwritten by  HDI_DEPLOY_OPTIONS`);
  }
