...
function prepareOptimiseMake (addedFiles, deletedFiles, modifiedFiles, unmodifiedFiles, options, content) {
  ...
  let filteredFiles;
  let deletedFilesNotInUndeployJSON = {};

  // filter the undeploy set based on the undeploy.json file
  if (undeployFiles.length && !options.autoUndeploy) {
    filteredFiles = filterUndeploy(undeployFiles, options);
    deletedFilesNotInUndeployJSON = undeployFiles.filter(file => !filteredFiles.includes(file));
    undeployFiles = filteredFiles;
  }
  logger.log('deleted files:', JSON.stringify(undeployFiles, null, 2));

  if (Array.isArray(deletedFilesNotInUndeployJSON)) {
    logger.WriteskippedDeletedFiles(deletedFilesNotInUndeployJSON);
  }
