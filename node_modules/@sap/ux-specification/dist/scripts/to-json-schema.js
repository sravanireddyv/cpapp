...
exports.convertInterfaces = void 0;
...
const compilerOptions = {
    skipLibCheck: true,
    types: ['node', '@sapui5/ts-types-esm', 'jest'],
...
function convertInterfaces(path, files, version) {
    const schemas = [];
    files.forEach((file) => {
        const program = typescript_json_schema_1.getProgramFromFiles([path_1.join(path, file)], compilerOptions, path);
        const currentType = file.replace('.ts', '');
        const generator = typescript_json_schema_1.buildGenerator(program, generatorSettings);
        if (generator) {
...
function writeSchemasToFile(path, schemas, version, eliminateVersion = false) {
    ...
    for (const name in schemas) {
        ...
        const file = path_1.join(dirToWrite, fileName);
...
async function toJsonSchema(outputPath) {
    ...
    const typesPath = path_1.join(rootName, 'types', 'src');
    //Pages V2
    let paths = [];
    paths.push(path_1.join(typesPath, 'v2', 'pages'));
    convertPagesOfFEVersion(paths, ux_specification_types_1.FioriElementsVersion.v2, outputPath);
    //Pages V4
    paths = [];
    paths.push(path_1.join(typesPath, 'v4', 'pages'));
    convertPagesOfFEVersion(paths, ux_specification_types_1.FioriElementsVersion.v4, outputPath);
    //App V2
    schemas = convertInterfaces(path_1.join(typesPath, 'v2'), ['ApplicationV2.ts'], ux_specification_types_1.FioriElementsVersion.v2);
    writeSchemasToFile(outputPath, schemas, 'v2');
    //App V4
    schemas = convertInterfaces(path_1.join(typesPath, 'v4'), ['ApplicationV4.ts'], ux_specification_types_1.FioriElementsVersion.v4);
