...
function addAdditionalFlexChanges(config, change, jsonSchema) {
    ...
    if ((!config.table.type || config.table.type === ux_specification_types_1.v2.TableTypeV2.ResponsiveTable) &&
        !flexForTransfer.controlId.includes(ux_specification_types_1.PAGETYPE_VIEW_EXTENSION_TEMPLATE_MAP.get(ux_specification_types_1.PageType.AnalyticalListPage))) {
...
function addFlex(config, changeString, appSchema, logger) {
    try {
        ...
        if (change.controlId?.includes(ux_specification_types_1.PageType.ListReport) ||
            change.controlId?.includes(ux_specification_types_1.PageType.AnalyticalListPage)) {
