...
function getPagesAndConfigs(v2Pages, flex, manifest, schemas, fragments, logger) {
    ...
    for (const v2PageName in v2Pages) {
        ...
        if (jsonSchema) {
            if (page.pageType === ux_specification_types_1.PageType.ListReport) {
            ...
            else if (page.pageType === ux_specification_types_1.PageType.AnalyticalListPage) {
                const importParameters = {
                    manifest,
                    flex,
                    jsonSchema: schemas[jsonSchema],
...
class V2AppProvider extends appProvider_1.AppProvider {
    constructor(manifest, changes, schemas, fragments, logger) {
        ...
        common_1.transferSettingsOfObject(appSettings, manifest, appSchema['definitions']['AppSettings'], undefined);
        // Call base
        super(manifest, schemas, generateId(v2Pages[pageKey]), getPagesAndConfigs(v2Pages, changes, manifest, schemas, fragments, logger), ux_specification_types_1.FioriElementsVersion.v2, appSettings);
    }
}
