...
function fillActions(importParameters, actionProperties, configPart, configParameters, importTypeId, facetId) {
    ...
    if (configParameters.flexChanges && Object.keys(configParameters.flexChanges).length > 0) {
        ...
        for (const actionProperty in actionProperties) {
            ...
            if (actionId) {
                configPart[ux_specification_types_1.PropertyName.actions][configActionId] = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, actionType);
...
function fillFields(importParameters, sectionPart, fieldProperties, configPart, configParameters) {
    ...
    for (const fieldProperty in fieldProperties) {
        ...
        configPart[sectionPart.id]['form']['fields'][fieldProperty] = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'Field');
...
function fillTableInConfig(importParameters, sectionPart, manifestSettings, configPart, configParameters) {
    ...
    if (manifestSection || configParameters.flexChanges) {
        ...
        const OPtable = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, `ObjectPage${tableType}`, {
            toolBar: {}
        });
        configPart[sectionPart.id] = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, sectionPart.type, {
            table: {}
        });
...
function fillCustomColumnsInConfig(importParameters, escapedSectionId, configPart, v2Page) {
    ...
    if (customColumnTarget) {
        ...
        common_2.addFragments(configPart[escapedSectionId], importParameters.manifest, v2Page, ux_specification_types_1.PageType.ObjectPage, importParameters.fragments, objectPageSectionData);
    }
}
/**
...
function fillChartSection(importParameters, sectionPart, manifestSettings, configPart, configParameters) {
    ...
    configPart[sectionPart.id][ux_specification_types_1.PropertyName.chart] = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'ObjectPageChart');
    if (manifestSection || configParameters.flexChanges) {
        transferSettingsAndFlexChanges(importParameters, configPart[sectionPart.id], sectionPart.schema, configParameters, { title: sectionPart.title });
...
function fillConfig(importParameters, configParameters) {
    const objectPageConfig = configParameters.factory.createPageInstance(ux_specification_types_1.PageType.ObjectPage);
    ...
    objectPageConfig.header = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, ux_specification_types_1.DefinitionName.ObjectPageHeader);
    ...
    const headerAction = definitions[ux_specification_types_1.DefinitionName.HeaderActions];
    if (headerAction) {
    ...
    objectPageConfig.layout = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'ObjectPageLayout');
    ...
    if (Object.keys(customSections).length > 0) {
        ...
        for (const sectionId in customSections) {
            const customSection = 'fragmentName' in customSections[sectionId]
                ? configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'ObjectPageCustomSectionFragment')
                : configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'ObjectPageCustomSectionView');
