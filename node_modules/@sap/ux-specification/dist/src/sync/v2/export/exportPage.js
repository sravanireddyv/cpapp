    ...
    if (currentSection && sectionId === 'custom') {
        ...
        for (const index in currentSection) {
            if (currentSection[index].className === ux_specification_types_1.SAPUI5_FRAGMENT_CLASS) {
                currentSection[index] = factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'ObjectPageCustomSectionFragment', currentSection[index]);
            }
            else {
                currentSection[index] = factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'ObjectPageCustomSectionView', currentSection[index]);
            }
        }
    }
}
...
function exportListReportPage(exportParameters, ui5Version) {
    ...
    const listReport = factory.createPageInstance(ux_specification_types_1.PageType.ListReport, listReportConfig);
...
function exportAnalyticalListPage(exportParameters, ui5Version) {
    ...
    const factory = new factory_1.MetadataInstanceFactoryV2();
    //Instantiate Page
    const analyticalListPage = factory.createPageInstance(ux_specification_types_1.PageType.AnalyticalListPage, analyticalListPageConfig);
    //Instantiate base ID
    ...
    if (analyticalListPageConfig && Object.keys(analyticalListPageConfig).length !== 0) {
        const transferParameters = {
            ...
            factory,
            pageKeys,
            pageType: ux_specification_types_1.PageType.AnalyticalListPage,
            logger: exportParameters.logger
        };
...
function exportObjectPage(exportParameters, ui5Version) {
    ...
    const exportResults = getDefaultExportResult(exportParameters.fragments);
    const factory = new factory_1.MetadataInstanceFactoryV2();
    const objectPage = factory.createPageInstance(ux_specification_types_1.PageType.ObjectPage, objectPageConfig);
    exportResults.manifest = JSON.parse(JSON.stringify(exportParameters.manifest));
    if (objectPageConfig) {
...
const exportOverviewPage = (exportParameters) => {
    ...
    exportResults.manifest = JSON.parse(JSON.stringify(exportParameters.manifest));
    const factory = new factory_1.MetadataInstanceFactoryV2();
    const overviewPage = factory.createPageInstance(ux_specification_types_1.PageType.OverviewPage, exportParameters.page.config);
